...
public class FTPopOverMenu: NSObject, FTPopOverMenuViewDelegate {
    ...
    fileprivate var popMenuHeight: CGFloat {
        return configuration.menuRowHeight * CGFloat(self.menuNameArray.count) + FT.DefaultMenuArrowHeight + (configuration.isTitleNeeded ? 34 : 0)
    }
...
fileprivate class FTPopOverMenuView: UIControl {
    ...
    @objc func buttonTapped(sender: UIButton) {
        ...
        self.delegate?.ftPopOverMenuClose()
    }
...
extension FTPopOverMenuView: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
        return (configuration.isTitleNeeded ? 34 : 0)
    }
    
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if !configuration.isTitleNeeded {
            return UIView(frame: .zero)
        }else{
            let frame: CGRect = tableView.frame
            
            let DoneBut: UIButton = UIButton(frame: CGRect(x: frame.size.width/2 + 25, y: 0, width: 34, height: 34)) //
            DoneBut.setImage(#imageLiteral(resourceName: "icon_close_gray"), for: .normal)
            DoneBut.backgroundColor = UIColor.clear
            
            DoneBut.addTarget(self, action: #selector(self.buttonTapped(sender:)), for: .touchUpInside)
                        
            let headerView: UIView = UIView(frame: CGRect(x: 0, y: 0, width: frame.size.width, height: 34))
...
extension FTPopOverMenuView: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if configuration.alternateCellColor {
            if indexPath.row % 2 == 0 {
                cell.backgroundColor = UIColor(red: 245/255, green: 245/255, blue: 245/255, alpha: 1.0)
            }else{
                cell.backgroundColor = UIColor(red: 221/255, green: 221/255, blue: 221/255, alpha: 1.0)
            }
        }
