...
- (void)computeOpenedSize
{
    ...
    [_items enumerateObjectsUsingBlock:^(UPStackMenuItem *item, NSUInteger idx, BOOL *stop) {
        height += item.frame.size.height + self->_itemsSpacing;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if([object isEqual:_baseButton]) {
        if([keyPath isEqualToString:@"center"]) {
            [_items enumerateObjectsUsingBlock:^(UPStackMenuItem *item, NSUInteger idx, BOOL *stop) {
                CGPoint convertedCenter = [item centerForItemCenter:self->_baseButton.center];
                [item setCenter:convertedCenter];
            }];
        }
    }
...
- (void)openStack
{
    ...
    [_items enumerateObjectsUsingBlock:^(UPStackMenuItem *item, NSUInteger idx, BOOL *stop) {
        NSTimeInterval duration = self->_openAnimationDuration;
        if(self->_animationType == UPStackMenuAnimationType_progressive)
            duration = ((idx + 1) * self->_openAnimationDuration) / itemsCount;
        else if(self->_animationType == UPStackMenuAnimationType_progressiveInverse)
            duration = ((itemsCount - idx) * self->_openAnimationDuration) / itemsCount;
        
        CGPoint center = CGPointMake(self->_baseButton.center.x, y + (item.frame.size.height/2 * way));
        CGPoint translatedCenter = [item centerForItemCenter:center];
        
        int64_t delay = idx * (self->_openAnimationDurationOffset*1000);
        dispatch_time_t time = dispatch_time(DISPATCH_TIME_NOW, (delay * NSEC_PER_MSEC));
        dispatch_after(time, dispatch_get_main_queue(), ^{
            [self moveItem:item toCenter:translatedCenter withDuration:duration opening:YES bouncing:self->_bouncingAnimation];
        });
        
        y += (item.frame.size.height + self->_itemsSpacing) * way;
    }];
...
- (void)closeStack
{
    ...
    [_items enumerateObjectsUsingBlock:^(UPStackMenuItem *item, NSUInteger idx, BOOL *stop) {
        NSTimeInterval duration = self->_closeAnimationDuration;
        if(self->_animationType == UPStackMenuAnimationType_progressive)
            duration = ((idx + 1) * self->_closeAnimationDuration) / itemsCount;
        else if(self->_animationType == UPStackMenuAnimationType_progressiveInverse)
            duration = ((itemsCount - idx) * self->_closeAnimationDuration) / itemsCount;
        
        CGPoint translatedCenter = [item centerForItemCenter:self->_baseButton.center];
        
        int64_t delay = idx * (self->_closeAnimationDurationOffset*1000);
        dispatch_time_t time = dispatch_time(DISPATCH_TIME_NOW, (delay * NSEC_PER_MSEC));
        dispatch_after(time, dispatch_get_main_queue(), ^{
            [self moveItem:item toCenter:translatedCenter withDuration:duration opening:NO bouncing:self->_bouncingAnimation];
        });
    }];
