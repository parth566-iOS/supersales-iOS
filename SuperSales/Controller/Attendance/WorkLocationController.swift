...
class WorkLocationController: BaseViewController {
    ...
    func setUI(){
        ...
        self.tf3.setCommonFeature()
        self.tfAddress.setCommonFeature()
        self.tfLocation.setCommonFeature()
        ...
        self.tf3.addBorders(edges: [.bottom], color: UIColor.black, cornerradius: 0)
        ...
        tf3.text = strAdd  //String.init(format:"\(selectedAddreeList.addressLine1),\(selectedAddreeList.addressLine2),\(selectedAddreeList.city),\(selectedAddreeList.state),\(selectedAddreeList.country)")
            ...
            //tfLocation.tag = self.
                            if(self.arrCustomerList.count > 0){
                                if let cust =  CustomerDetails.getCustomerByID(cid: dsetting?.clientvendorid ?? 0){
                               AttendanceViewController.arrOfSelectedMultipleCustomer = [cust]
                                }else{
        ...
        let longdouble = address.longitude as? Double ?? 0.00
            ...
            tfAddress.tag = self.getVendorIndex(vendorId: dsetting?.clientvendorid.intValue ?? 0)
                        ...
                        tf3.tag = self.getAddressFromVendor(ven: vendor, addressId: dsetting?.addressID as! NSInteger)
                            ...
                            if(self.arrVendorList.count > 0){
                                if let cust =  Vendor().getVendorById(aId: dsetting?.clientvendorid ?? 0){
                                    AttendanceViewController.arrOfSelectedVendor = [cust]
                                }else{
                                    AttendanceViewController.arrOfSelectedVendor = [self.arrVendorList[0]]
                                }
                            }
                            }
...
//                        Utils.toastmsg(message:"You are not allowdwd checkin from Home")
...
//            }
            ...
            SVProgressHUD.setDefaultMaskType(.black)
        SVProgressHUD.show()
            self.apihelper.getdeletejoinvisit(param: param, strurl: ConstantURL.kWSUrlSetDefaultSetting, method: Apicallmethod.post) { (totalpages,pagesavailable,lastsynctime,arr,status,message,error,responseType) in
            SVProgressHUD.dismiss()
                if(status.lowercased() == Constant.SucessResponseFromServer){
                    if ( message.count > 0 ) {
                     Utils.toastmsg(message:message,view: self.view)
